...
void update_accel_est(accel_est* obj, double t, double ay) {
    ...
    obj->AccBodyY[obj->i - 1] = (ay - 1) * -1 * obj->g;  // convert to m/s^2 and correct 1g
    ...
    if (obj->i <= 1) {
        ...
        obj->VelDown[obj->i - 1] = obj->VelDown[obj->i - 2] + (timestep[1] * (obj->AccDown[obj->i - 2] + obj->AccDown[obj->i - 1]) / 2);
        obj->PosDown[obj->i - 1] = obj->PosDown[obj->i - 2] + (timestep[1] * (obj->VelDown[obj->i - 2] + obj->VelDown[obj->i - 1]) / 2);
    }
