...
%% Integration time!
tMin = 160; % only before deployment 
iMin = interp1(flight.Timestamp,1:length(flight.Timestamp),tMin,'nearest');

tMax = 32170; % only before deployment 
iMax = interp1(flight.Timestamp,1:length(flight.Timestamp),tMax,'nearest');
...
[velocity, position] = TrapInt(flight.Timestamp(iMin:iMax) / 1000, ...
    flight.Ax(iMin:iMax)*g-g, flight.Ay(iMin:iMax)*g, flight.Az(iMin:iMax)*g);
...
for c = 1:3
    varName = names(c);
    plot(flight.Timestamp(iMin+1:iMax)/1000, velocity(:,c), "Displayname", char(varName))
end
hold off
xlabel("time (s)")
ylabel("velocity (m/s)")
...
for c = 1:3
    varName = names(c);
    plot(flight.Timestamp(iMin+1:iMax-1)/1000, position(:,c), "Displayname", char(varName))
end
xlabel("time (s)")
